...
class DOCSTATUS(Base):
    __tablename__ = 'DOCSTATUS'
    id = Column(Integer, primary_key=True, autoincrement=True)
    status_name = Column(VARCHAR(100))
    document = relationship("DOCUMENTS", back_populates='status')


class DOCUMENTS(Base):
    __tablename__ = 'DOCUMENTS'
    id = Column(Integer, primary_key=True, autoincrement=True)
    status_id = Column(Integer, ForeignKey("DOCSTATUS.id"))
    filename = Column(VARCHAR(500))
    path2doc = Column(VARCHAR(1000))
    date = Column(DateTime, name='Date')

    status = relationship("DOCSTATUS", back_populates="document")
    paragraphs = relationship("PARAGRAPH", back_populates="document", cascade='all, delete')
...
class PARAGRAPH(Base):
    ...
    text = Column(BLOB)
    document = relationship("DOCUMENTS", back_populates="paragraphs")

    telephones = relationship("TELEPHONES", back_populates="paragraph", cascade='all, delete')
    surnames = relationship("SURNAMES", back_populates="paragraph", cascade='all, delete')
...
class TELEPHONES(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    paragraph_id = Column(Integer, ForeignKey("PARAGRAPH.id", ondelete="CASCADE"))
    number = Column(VARCHAR(50))
    number_integer = Column(BigInteger, nullable=False)
    paragraph = relationship("PARAGRAPH", back_populates="telephones")
...
class SURNAMES(Base):
    ...
    paragraph = relationship("PARAGRAPH", back_populates="surnames")
...
class NumbersView(Base):
    __tablename__ = 'NUMBERS_VIEW'
    id = Column(Integer, primary_key=True)
    numbers = Column(VARCHAR(50))
    numbers_int = Column(BigInteger)
    text = Column(Text)
    filename = Column(VARCHAR(500))
...
class SurnamesView(Base):
    __tablename__ = 'SURNAMES_VIEW'
    id = Column(Integer, primary_key=True)
    name = Column(VARCHAR(100))
    patronymic = Column(VARCHAR(100))
    surname = Column(VARCHAR(100))
...
class DataBase:
    ...
    def __init__(self, path2db, server_ip, port='3050', login='sysdba', password='masterkey'):
        self.port: int = int(port)
        self.server_ip = server_ip
        self.connect_string = f"firebird+fdb://{login}:{password}@{server_ip}:{port}/{path2db}?charset=UTF8"
    ...
    def __generation_numbers_requestion(numbers_list):
        ...
        for number in numbers_list:
            numbers_requestion_list.append(TELEPHONES(number))
    ...
    def __generation_personal_requestion(personal_list):
        ...
        for personal in personal_list:
            personal_requestions_list.append(SURNAMES(personal['first'], personal['middle'], personal['last']))
    ...
    def __generate_paragraph_list(self, paragraphs, number_map, persons_map):
        ...
        for item in range(0, len(paragraphs)):
            if len(number_map[item]) or len(persons_map[item]):
                paragraph = PARAGRAPH(text=paragraphs[item])
    ...
    def add_record(self, filename, filepath, status, number_map, paragraphs, persons_map, doc_date):
        document = DOCUMENTS(filename=filename, path=filepath, status_id=status, date=doc_date)
...
if __name__ == '__main__':
    ...
    page = 1
    db = DataBase('/home/gravity/Temp/Base/db_numbers_doc_status.fdb', '127.0.0.1')
    db.try_connection()
    # db.create_tables()
    db.test_select()
    # db.test_insert()
